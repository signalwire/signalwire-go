// Package config holds the JSON configuration loader mechanics shared by
// pkg/agent (which re-exports them as the public agent.ConfigLoader, the Go
// spelling of signalwire.core.config_loader.ConfigLoader) and pkg/security
// (which needs the same loader for SecurityConfig's config-file layer).
//
// It lives under internal/ purely to break an import cycle: pkg/agent already
// imports pkg/security, so pkg/security cannot import pkg/agent. Neither
// duplicating the loader nor moving the public type out of pkg/agent is
// acceptable — the first would let the two copies drift, the second would move
// public surface — so the mechanics live here and pkg/agent stays the façade.
package config
...
// Loader loads JSON configuration with ${VAR|default} environment-variable
...
// New creates a Loader. When configPaths is nil the default search paths are
// used. The first existing, parseable file wins.
func New(configPaths []string) *Loader {
	c := &Loader{configPaths: configPaths}
	if c.configPaths == nil {
		c.configPaths = DefaultPaths()
	}
	c.load()
	return c
}

// DefaultPaths returns the default configuration search paths (mirrors the
...
func (c *Loader) ConfigPaths() []string {
	return append([]string(nil), c.configPaths...)
}

// HasConfig reports whether a configuration file was loaded.
func (c *Loader) HasConfig() bool { return c.config != nil }

// ConfigFile returns the path of the loaded config file, or "" if none.
func (c *Loader) ConfigFile() string { return c.configFile }

// Config returns the raw configuration (before variable substitution).
func (c *Loader) Config() map[string]any {
	if c.config == nil {
...
func (c *Loader) Get(keyPath string, def any) any {
	if c.config == nil {
		return def
	}
...
// empty map if absent.
func (c *Loader) GetSection(section string) map[string]any {
	if c.config == nil {
		return map[string]any{}
	}
...
// → ssl.enabled).
func (c *Loader) MergeWithEnv(envPrefix string) map[string]any {
