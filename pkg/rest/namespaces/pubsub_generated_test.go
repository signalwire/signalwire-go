...
func TestPubsubGen_PubSub_CreateToken(t *testing.T) {
	t.Parallel()
	...
	mock.Reset(t)
	_, err := client.PubSub.CreateToken(nil, nil, nil, nil, map[string]any{"ttl": "x-1", "channels": "x-1"})
	if err != nil {
		t.Fatalf("call: %v", err)
	}
	j := mock.Last(t)
...
func TestPubsubGen_PubSub_CreateToken_Error(t *testing.T) {
	t.Parallel()
	client, mock := mocktest.New(t)
	if client == nil {
		return
