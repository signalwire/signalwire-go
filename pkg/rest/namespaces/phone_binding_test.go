...
func (m *callRecorder) Get(_ context.Context, path string, params map[string]string, _ ...*RequestOptions) (map[string]any, error) {
	m.record(recordedCall{Method: "GET", Path: path, Params: params})
	return m.resp, nil
}
func (m *callRecorder) Post(_ context.Context, path string, body map[string]any, params map[string]string, _ ...*RequestOptions) (map[string]any, error) {
	m.record(recordedCall{Method: "POST", Path: path, Body: body, Params: params})
	return m.resp, nil
}
func (m *callRecorder) Put(_ context.Context, path string, body map[string]any, _ ...*RequestOptions) (map[string]any, error) {
	m.record(recordedCall{Method: "PUT", Path: path, Body: body})
	return m.resp, nil
}
func (m *callRecorder) Patch(_ context.Context, path string, body map[string]any, _ ...*RequestOptions) (map[string]any, error) {
	m.record(recordedCall{Method: "PATCH", Path: path, Body: body})
	return m.resp, nil
}
func (m *callRecorder) Delete(_ context.Context, path string, _ ...*RequestOptions) (map[string]any, error) {
...
func TestPhoneNumbers_Update_UsesPUT(t *testing.T) {
	...
	_, _ = pn.Update(context.Background(), "pn-1", map[string]any{"name": "Main"})
	calls := mock.Calls()
	if len(calls) != 1 {
...
func TestSetSwmlWebhook_HappyPath(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.SetSwmlWebhook(context.Background(), "pn-1", "https://example.com/swml", nil)
	calls := mock.Calls()
	if len(calls) != 1 {
		t.Fatalf("calls = %d, want 1", len(calls))
	}
...
func TestSetSwmlWebhook_Extra(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.SetSwmlWebhook(context.Background(), "pn-1", "https://example.com/swml", nil, map[string]any{
		"name": "Support Line",
	})
...
func TestSetCxmlWebhook_Minimal(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.SetCxmlWebhook(context.Background(), "pn-1", "https://example.com/voice.xml", nil, nil, nil)
	body := mock.Calls()[0].Body
...
func TestSetCxmlWebhook_WithFallbackAndStatus(t *testing.T) {
	...
	_, _ = pn.SetCxmlWebhook(context.Background(), "pn-1", "https://example.com/voice.xml", &fallback, &status, nil)
	body := mock.Calls()[0].Body
	want := map[string]any{
...
func TestSetCxmlApplication_HappyPath(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.SetCxmlApplication(context.Background(), "pn-1", "app-1", nil)
	body := mock.Calls()[0].Body
...
func TestSetAiAgent_HappyPath(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.SetAiAgent(context.Background(), "pn-1", "agent-1", nil)
	body := mock.Calls()[0].Body
...
func TestSetCallFlow_Minimal(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.SetCallFlow(context.Background(), "pn-1", "cf-1", nil, nil)
	body := mock.Calls()[0].Body
...
func TestSetCallFlow_WithVersion(t *testing.T) {
	...
	_, _ = pn.SetCallFlow(context.Background(), "pn-1", "cf-1", &version, nil)
	body := mock.Calls()[0].Body
...
func TestSetRelayApplication_HappyPath(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.SetRelayApplication(context.Background(), "pn-1", "my-app", nil)
	body := mock.Calls()[0].Body
...
func TestSetRelayTopic_Minimal(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.SetRelayTopic(context.Background(), "pn-1", "office", nil, nil)
	body := mock.Calls()[0].Body
...
func TestSetRelayTopic_WithStatusCallback(t *testing.T) {
	...
	_, _ = pn.SetRelayTopic(context.Background(), "pn-1", "office", &statusCB, nil)
	body := mock.Calls()[0].Body
...
func TestBindingRegression_WireLevelFormStillWorks(t *testing.T) {
	pn, mock := newPhoneNumbers()
	_, _ = pn.Update(context.Background(), "pn-1", map[string]any{
