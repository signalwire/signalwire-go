...
security.ValidateRequestE: Go error-returning variant of request-signature validation (the `E` suffix is go's idiom for the error-returning form alongside the bool-returning one). Not oracle surface
security.ValidateWebhookSignatureE: Go error-returning variant of webhook-signature validation (`E` = go's error-returning form). Not oracle surface
security.WebhookMiddleware: Go net/http middleware that validates an inbound webhook signature before the handler runs — the go-idiomatic form of Python's request-signature validation applied per-route. Not oracle surface
