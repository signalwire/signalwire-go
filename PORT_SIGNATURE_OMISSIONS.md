...
## AI-Chat client: functional-options + struct-literal idioms (signalwire.ai_chat.client)

signalwire.ai_chat.client.AIChatClient.__init__: go-factory-ctor + go-idiom-options-collapse — Go constructs via NewClient(opts ...Option), collapsing Python's project/token/space/url/session kwargs into functional options (WithProject/WithToken/WithSpace/WithURL/WithHTTPClient/WithReadIdleTimeout); same Basic-auth + endpoint resolution, wire unchanged.
signalwire.ai_chat.client.AIChatClient.create_conversation: go-idiom-options-collapse — Go CreateConversation(ctx context.Context, conversationID string, opts CreateOptions) threads a leading context.Context (Go's cancellation primitive) and collapses Python's config_url/user_message/timeout/user_metadata/reinit keyword args into the CreateOptions struct; same conversation.create JSON-RPC params.
signalwire.ai_chat.client.AIChatClient.chat: go-idiom-options-collapse — Go Chat(ctx context.Context, conversationID, message string, opts ...ChatOptions) threads a leading context.Context and collapses Python's role/config_url/timeout/user_metadata/reinit keyword args into the variadic ChatOptions struct; same conversation.chat JSON-RPC params.
signalwire.ai_chat.client.AIChatClient.summarize: go-idiom-options-collapse — Go Summarize(ctx context.Context, conversationID string, opts ...SummarizeOptions) collapses Python's summary_prompt keyword arg into the variadic SummarizeOptions struct (SummaryPrompt field); same conversation.summary JSON-RPC params.
signalwire.ai_chat.client.AIChatError.__init__: go-struct-literal — Go's AIChatError is a value struct (Code/HasCode/Message public fields) built by the unexported newTypedError(code, message) helper / a composite literal, so there is no exported constructor to project as __init__; the reference's (code, message) __init__ maps onto the two public fields.
signalwire.ai_chat.client.ConversationInfo.__init__: go-struct-literal — Go's ConversationInfo is a plain value struct (ID/Status/InitialMessage public fields) constructed by a composite literal from the decoded response; no NewX factory exists to project as the reference's generated dataclass __init__.
signalwire.ai_chat.client.ChatResponse.__init__: go-struct-literal — Go's ChatResponse is a plain value struct (Text/ConversationID/UserEvent public fields) constructed by a composite literal from the decoded response; no NewX factory exists to project as the reference's generated dataclass __init__.
signalwire.ai_chat.client.ChatLog.__init__: go-struct-literal — Go's ChatLog is a plain value struct constructed by a composite literal from the decoded response; no NewX factory exists to project as the reference's generated dataclass __init__.
